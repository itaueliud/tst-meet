'use client';
import { Mic, MicOff, Video, VideoOff, MonitorUp, MonitorOff, Hand, MessageSquare, Users, Layout, Maximize, PhoneOff, Lock, Unlock, PenTool } from 'lucide-react';
...
export default function MeetingControls({
  ...
  return (
    <div className="relative flex items-center justify-between px-4 py-3 glass border-t border-white/10">
      {/* Left: info */}
      ...
      <div className="flex items-center gap-2 justify-center">
        ...
        {/* Camera */}
        <button onClick={onToggleCamera} title={isCameraOff ? 'Turn on camera' : 'Turn off camera'}
          className={`control-btn ${isCameraOff ? 'muted' : 'active'}`}>
          {isCameraOff ? <VideoOff className="w-5 h-5 text-white" /> : <Video className="w-5 h-5 text-white" />}
        </button>

        {/* Screen share */}
        <button onClick={isSharingScreen ? onStopShare : onShareScreen}
          title={isSharingScreen ? 'Stop sharing' : 'Share screen'}
          className={`control-btn ${isSharingScreen ? 'bg-blue-600 hover:bg-blue-700' : 'active'}`}>
        ...
        <button onClick={onToggleHand}
          className={`control-btn ${handRaised ? 'bg-yellow-500 hover:bg-yellow-600' : 'active'}`}
        ...
        <div className="relative">
          <button onClick={() => setShowEmojiPicker(!showEmojiPicker)}
            className="control-btn active text-lg">😀</button>
          {showEmojiPicker && (
            <div className="absolute bottom-14 left-1/2 -translate-x-1/2 bg-slate-800 border border-white/10 rounded-xl p-2 flex gap-1.5 shadow-2xl z-50">
              {EMOJIS.map(e => (
                <button key={e} onClick={() => { onSendReaction(e); setShowEmojiPicker(false); }}
                  className="text-2xl hover:scale-125 transition-transform w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white/10">
                  {e}
                </button>
              ))}
            </div>
        ...
        {isHost ? (
          <button onClick={onEndMeeting}
            className="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2.5 rounded-full flex items-center gap-2 transition-colors text-sm">
      ...
      <div className="flex items-center gap-1.5 justify-end w-1/4">
        {/* Lock (host only) */}
        {isHost && (
          <button onClick={() => onLockMeeting?.(!isLocked)}
            className={`control-btn ${isLocked ? 'bg-orange-600 hover:bg-orange-700' : 'active'}`}
        ...
        <button onClick={() => setShowWhiteboard(!showWhiteboard)}
          className={`control-btn ${showWhiteboard ? 'bg-blue-600' : 'active'}`} title="Whiteboard">
          <PenTool className="w-4 h-4 text-white" />
        </button>

        {/* Participants */}
        <button onClick={() => setShowParticipants(!showParticipants)}
          className={`control-btn ${showParticipants ? 'bg-blue-600' : 'active'}`} title="Participants">
          <Users className="w-4 h-4 text-white" />
        </button>

        {/* Chat */}
        <button onClick={() => { setShowChat(!showChat); }}
          className={`control-btn relative ${showChat ? 'bg-blue-600' : 'active'}`} title="Chat">
        ...
        {/* View toggle */}
        <button onClick={() => setView(view === 'grid' ? 'speaker' : 'grid')}
          className="control-btn active" title="Toggle view">
