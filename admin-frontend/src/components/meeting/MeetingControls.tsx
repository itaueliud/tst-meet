        ...
        <div className="relative">
          ...
          {showEmojiPicker && (
            <div className="absolute bottom-14 left-1/2 -translate-x-1/2 bg-slate-800 border border-white/10 rounded-xl p-2 flex gap-1.5 shadow-2xl z-50">
              {EMOJIS.map(e => (
                <button key={e} onClick={() => { onSendReaction(e); setShowEmojiPicker(false); }} className="text-2xl hover:scale-125 transition-transform w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white/10">
                  {e}
                </button>
              ))}
            </div>
